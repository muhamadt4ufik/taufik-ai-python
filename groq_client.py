...
class GroqClient:
    ...
    def send_chat_non_stream(
        ...
        candidate_models = [model] + [m for m in AVAILABLE_MODELS if m != model] if model else AVAILABLE_MODELS
        ...
        for candidate in candidate_models:
            ...
            try:
                ...
                if response.status_code != 200:
                    ...
                    attempt_errors.append(f"{candidate}: {err_msg}")
                    if response.status_code == 401:
                ...
                attempt_errors.append(f"{candidate}: Gagal menghubungi Groq ({e})")

        if not success:
            detail = " | ".join(attempt_errors) if attempt_errors else "Tidak diketahui"
            raise RuntimeError(f"Gagal memuat respons: {detail}")
